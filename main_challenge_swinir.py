...
#    print(torch.backends.cudnn.version())  # cudnn version
    ...
    # basic settings
    # --------------------------------
    model_names = ['msrresnet', 'imdn', 'swinir', 'swinir-lite']
    model_id = 3                  # set the model name
    sf = 4
    ...
    if model_name == 'msrresnet':
        ...
        model = net(upscale=sf, in_chans=3, img_size=training_patch_size, window_size=8,
                    img_range=1., depths=[6, 6, 6, 6, 6, 6], embed_dim=180, num_heads=[6, 6, 6, 6, 6, 6],
                    mlp_ratio=2, upsampler='pixelshuffle', resi_connection='1conv')
        model_path = os.path.join('model_zoo', 'swinir', '001_classicalSR_DF2K_s64w8_SwinIR-M_x4.pth')            # set model path
        param_key_g = 'params'
    
    elif model_name == 'swinir-lite':
        from models.network_swinir import SwinIR as net
        model = net(upscale=sf, in_chans=3, img_size=64, window_size=8,
                    img_range=1., depths=[6, 6, 6, 6], embed_dim=60, num_heads=[6, 6, 6, 6],
                    mlp_ratio=2, upsampler='pixelshuffledirect', resi_connection='1conv')
        model_path = os.path.join('model_zoo', 'swinir', '002_lightweightSR_DIV2K_s64w8_SwinIR-S_x4.pth')   
        param_key_g = 'params'
